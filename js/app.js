...
var Application = /*#__PURE__*/function () {
  ...
  return _createClass(Application, [{
    ...
    key: "deliverablesInit",
    value: function deliverablesInit() {
      var t = this;
      var $doc = this.$doc;
      $doc.on('click', '.deliverables-menu__item', function (e) {
        ...
        var $container = $t.closest('.deliverables-container');
        var $arrow = $container.find('.deliverables-menu__icon');
        ...
        var $linkTopPosition = $t.offset().top;
        var $linkH = $t.outerHeight();
        var pos = $linkTopPosition - $containerTopPosition;
        pos = pos + 16;
        var rem = parseInt($doc.find('html').css('font-size'));
        if (isNaN(rem)) {
      ...
      function myResizeFunction() {
        ...
        $doc.find('.deliverables-container').each(function () {
          var $container = $(this);
          var $t = $container.find('.deliverables-menu__item.active');
          var $arrow = $container.find('.deliverables-menu__icon');
          if ($arrow.length === 0) return;
          var $containerTopPosition = $container.offset().top;
          var $linkTopPosition = $t.offset().top;
      ...
      var debouncedResizeHandler = debounce(myResizeFunction, 200);
    ...
    value: function linkListener() {
      var t = this;
      var $doc = this.$doc;
      (0,_ui_request_on_click__WEBPACK_IMPORTED_MODULE_9__.sendRequestClickListener)();
      (0,_ui_tabs__WEBPACK_IMPORTED_MODULE_8__.tabs)();
      ...
      $doc.on('click', '.header-container-mobile .menu-item-has-children', function (e) {
        ...
        var $t = $(this);
        var $wrapper = $t;
        var $container = $('.header-container-mobile .sub-container');
...
/* harmony export */   initEventsListener: () => (/* binding */ initEventsListener),
...
var openModal = function openModal($el) {
  ...
  var $closeButton = $el.find('.close-custom-modal');
...
var CustomSlider = /*#__PURE__*/function () {
  ...
  return _createClass(CustomSlider, [{
    ...
    key: "cloneElements",
    value: function cloneElements($slider) {
      var cloneCount = $slider.attr('data-clone') || 0;
      cloneCount = parseInt(cloneCount);
      if (isNaN(cloneCount)) return;
      if (cloneCount <= 0) return;
      var html = $slider.html();
      var newHTML = '';
      for (var a = 1; a <= cloneCount; a++) {
        newHTML += html;
      }
      $slider.html(newHTML);
    }
    ...
    value: function addDots($slider) {
      ...
      var $wrapper = $slider.closest('section').find('.custom-slider-wrapper');
      if ($wrapper.length === 0) {
    ...
    value: function init() {
      ...
      $(document).on('click', '.custom-slider__dot', function (e) {
        e.preventDefault();
        var $t = $(this);
        var $slider = $t.closest('.section').find('.custom-slider');
        var gap = $slider.css('gap');
        gap = parseInt(gap);
        gap = isNaN(gap) ? 0 : gap;
        var currentIndex = _this.getCurrentIndex($slider);
        var index = $t.index();
        _this.movement($slider, index, gap);
      });
    }
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunkconsulting"] = self["webpackChunkconsulting"] || [];
